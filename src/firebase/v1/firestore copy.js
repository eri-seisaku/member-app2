...
// 配列から特定の要素を削除する関数
export async function removeElementFromArray(collectionName, userID, arrayField, elementsToRemoveObj) {
  const docRef = doc(db, collectionName, userID);

  // オブジェクトから値だけを取り出して配列に変換
  const elementsToRemove = Object.values(elementsToRemoveObj);

  await updateDoc(docRef, {
    [arrayField]: arrayRemove(...elementsToRemove)
  });
}

// 配列から特定の要素を追加する関数
export async function addElementToArray(collectionName, userID, arrayField, element) {
  const docRef = doc(db, collectionName, userID);
